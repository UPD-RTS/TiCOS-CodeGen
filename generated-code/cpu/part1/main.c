#include "activity.h"
#include <arinc653/types.h>
#include <arinc653/process.h>
#include <arinc653/partition.h>
#include "deployment.h"
#include <arinc653/sampling.h>
#include <arinc653/queueing.h>
#include <arinc653/blackboard.h>


PROCESS_ID_TYPE arinc_threads[POK_CONFIG_NB_THREADS];

extern BLACKBOARD_ID_TYPE pok_blackboards_ids[1];
extern char* pok_blackboards_names[POK_CONFIG_NB_BLACKBOARDS];
// Sampling ports ids
SAMPLING_PORT_ID_TYPE SS_0_id;
// Queuing ports ids
QUEUING_PORT_ID_TYPE QD_1_id;

int main ()
{

	PROCESS_ATTRIBUTE_TYPE tattr;
	RETURN_CODE_TYPE ret;
	uint8_t blackboards_max_message_size = 256;

	printf("	part1 - Main thread\n");
	// Sampling ports creation 
	CREATE_SAMPLING_PORT ("port_01", POK_CONFIG_MAX_MESSAGE_SIZE, SOURCE, 2000, &(SS_0_id), &(ret));
	printf("	Sampling port created! id: 1, name: port_01, core id:%d, ret:%d\n", SS_0_id, ret);

	// Queuing ports creation 
	CREATE_QUEUING_PORT ("port_02", POK_CONFIG_MAX_MESSAGE_SIZE,  POK_CONFIG_PART_MAX_QUEUING_MESSAGES, DESTINATION, 0, &(QD_1_id), &(ret));
	printf("	Queueing port created! id: 2, name: port_02, core id:%d, ret:%d\n", QD_1_id, ret);


	CREATE_BLACKBOARD (pok_blackboards_names[0], blackboards_max_message_size, &(pok_blackboards_ids[0]), &(ret));
	printf("	Blackboard %s created! BLACKBOARD ID: %u, ret: %d \n", pok_blackboards_names[0], pok_blackboards_ids[0], ret);

	tattr.PERIOD = 200;
	tattr.BASE_PRIORITY = 19;
	tattr.ENTRY_POINT = thr1_1_job;
	CREATE_PROCESS (&(tattr), &(arinc_threads[1]), &(ret));

	tattr.PERIOD = APERIODIC_PROCESS;
	tattr.BASE_PRIORITY = 18;
	tattr.ENTRY_POINT = thr1_1bis_job;
	CREATE_PROCESS (&(tattr), &(arinc_threads[2]), &(ret));

	tattr.PERIOD = 200;
	tattr.BASE_PRIORITY = 17;
	tattr.ENTRY_POINT = thr1_3_job;
	CREATE_PROCESS (&(tattr), &(arinc_threads[3]), &(ret));

	tattr.PERIOD = APERIODIC_PROCESS;
	tattr.BASE_PRIORITY = 16;
	tattr.ENTRY_POINT = thr1_3bis_job;
	CREATE_PROCESS (&(tattr), &(arinc_threads[4]), &(ret));

	tattr.PERIOD = 200;
	tattr.BASE_PRIORITY = 7;
	tattr.ENTRY_POINT = thr1_5_job;
	CREATE_PROCESS (&(tattr), &(arinc_threads[5]), &(ret));

	tattr.PERIOD = 200;
	tattr.BASE_PRIORITY = 6;
	tattr.ENTRY_POINT = thr1_6_job;
	CREATE_PROCESS (&(tattr), &(arinc_threads[6]), &(ret));


	START (arinc_threads[1],&(ret));
	START (arinc_threads[2],&(ret));
	START (arinc_threads[3],&(ret));
	START (arinc_threads[4],&(ret));
	START (arinc_threads[5],&(ret));
	START (arinc_threads[6],&(ret));

	SET_PARTITION_MODE (NORMAL, &(ret));
	return 0;
}
